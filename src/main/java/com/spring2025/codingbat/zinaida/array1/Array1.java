...
the original array, and all the other elements are 0. The original array will be length 1 or more. Note: by default, a new int array contains all 0's.
...
maxTriple([5, 2, 3]) → 5
    ...
    public int maxTriple(int[] nums) {
        ...
        if (nums[nums.length / 2] > nums[nums.length - 1] && nums[0] < nums[nums.length / 2]) {
...
unlucky1([1, 1, 1]) → false
 ...
 followed by the elements from b. The arrays may be any length, including 0, but there will be 2 or more elements available between the 2 arrays.
